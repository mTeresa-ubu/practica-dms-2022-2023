...
def create_comment(respuestaId: int, body: Dict, token_info: Dict) -> Tuple[Union[Dict,str],int]:
    """ Crea un comentario
    """
    with current_app.app_context():
        usuario = token_info['user_token']['username']
        comentario = servicioComentario.crear_comentario(current_app.db, usuario, body['body'], respuestaId, body['sentiment'])
...
def get_comments(aid:int) -> Tuple[Dict,int]:
    ...
    """
    with current_app.app_context():
        comentarios:servicioComentario.get_comentarios(current_app.db,aid)
